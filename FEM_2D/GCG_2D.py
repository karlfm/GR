...
def setup_problem(mesh, params):
    ...
    mandel_trace = dolfinx.fem.Function(scalar_space, name="mandel_trace")
...
def weak_formulation(mesh, facet_tags, params, problem_variables, QUAD_DEGREE=8):
    ...
    mandel_trace = problem_variables["mandel_trace"]
    ...
    expressions = {
        ...
        "mandel_trace": dolfinx.fem.Expression(  # hoop strain
        ufl.tr(ufl.det(F) * cauchy),
        scalar_space.element.interpolation_points(),
        ),
        ...
        "dgt_expr": dolfinx.fem.Expression(params["dt"] * (mandel_trace - params["set_point"]), scalar_space.element.interpolation_points()),

        "g2_expr": dolfinx.fem.Expression(g_t + params["dt"] * (mandel_trace - params["set_point"]), scalar_space.element.interpolation_points()),
    }
...
def run_simulation(params, problem_variables, weak_form_defs, data_collector):
    ...
    mandel_trace.interpolate(expressions["mandel_trace"])
...
def main():
    ...
    data_collector.register_function("Trace of Mandel Stress", problem_vars["mandel_trace"])
