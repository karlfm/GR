...
def weak_formulation(mesh, facet_tags, params, problem_variables, QUAD_DEGREE=8):
    ...
    expressions = {
        ...
        "dgr_expr": dolfinx.fem.Expression(pow((params["dt"] * (ufl.sqrt(2*elastic_strain_rr + 1) - 1 - params["set_point"]) + 1), 1/3), scalar_space.element.interpolation_points()),
        "dgt_expr": dolfinx.fem.Expression(pow((params["dt"] * (ufl.sqrt(2*elastic_strain_ff + 1) - 1 - params["set_point"]) + 1), 1/3), scalar_space.element.interpolation_points()),

        "gr_expr": dolfinx.fem.Expression(g_r * pow((params["dt"] * (ufl.sqrt(2*elastic_strain_rr + 1) - 1 - params["set_point"]) + 1), 1/3), scalar_space.element.interpolation_points()),
        "gt_expr": dolfinx.fem.Expression(g_t * pow((params["dt"] * (ufl.sqrt(2*elastic_strain_ff + 1) - 1 - params["set_point"]) + 1), 1/3), scalar_space.element.interpolation_points()),
    }
...
def main():
    ...
    data_collector.register_function("Elastic Hoop Strain", problem_vars["elastic_strain_ff"])
