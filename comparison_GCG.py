...
problem_vars = FEM_2D.GCG_2D.setup_problem(mesh, params)
weak_form_defs = FEM_2D.GCG_2D.weak_formulation(mesh, facet_tags, params, problem_vars)

data_collector = FEM_2D.GCG_2D.DataCollector(comm, output_dir, line_points)
...
data_collector.register_line_data("Trace of Mandel Stress", problem_vars["mandel_trace"])
...
for i, state in enumerate(states):
    ri_1d = state.find_inner_radius()
    mandel_trace_1d = np.array([state.mandel_trace(ri_1d, s) for s in r_range])
    axs[1,2].plot(r_range, mandel_trace_1d, color='C0', alpha=(i+1)/(num_steps + 1), label=f"1D (ODE) {i}")
    mandel_trace_2d = data_collector.line_history.get("Trace of Mandel Stress")[i].flatten()
    axs[1,2].plot(r_range, mandel_trace_2d, color='C1', linestyle='--', alpha=(i+1)/(num_steps + 1), label=f"2D (FEM) {i}")
axs[1,2].set_title("Trace of Mandel Stress") 
