...
problem_vars = FEM_2D.GEG_2D.setup_problem(mesh, params)
weak_form_defs = FEM_2D.GEG_2D.weak_formulation(mesh, facet_tags, params, problem_vars)

data_collector = FEM_2D.GEG_2D.DataCollector(comm, output_dir, line_points)
...
for i, state in enumerate(states):
    ri_1d = state.find_inner_radius()
    hoop_strain_1d = np.array([state.compute_hoop_strain(ri_1d, s) for s in r_range])
    axs[1,2].plot(r_range, hoop_strain_1d, color='C0', alpha=(i+1)/(num_steps + 1), label=f"1D (ODE) {i}")
    hoop_strain_2d = data_collector.line_history.get("Hoop Strain")[i].flatten()
